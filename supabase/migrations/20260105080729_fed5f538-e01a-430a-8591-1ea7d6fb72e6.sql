-- Corrigir a função create_customer_with_auth (9 parâmetros) para usar extensions.crypt e extensions.gen_salt
-- E também criar como REVENDA com status TRIAL quando p_trial_days > 0
...
BEGIN
  ...
  INSERT INTO customers (
    ...
    CASE WHEN p_trial_days IS NOT NULL AND p_trial_days > 0 THEN 'active' ELSE 'pending' END
  ...
  -- 4. Criar usuário no auth.users - USANDO extensions.crypt e extensions.gen_salt
  ...
  -- 8. Criar notificação para o tenant dono (apenas se for cliente pendente)
  IF v_tenant_status = 'pending' THEN
  ...
  RETURN json_build_object(
    ...
    'user_id', v_user_id,
    'is_trial', p_trial_days IS NOT NULL AND p_trial_days > 0
  );
