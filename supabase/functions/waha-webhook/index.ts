  ...
  try {
    ...
    if (event === 'message') {
      ...
      if (isGroupMessage) {
        ...
        // Se o grupo existe no banco, verificar suas configurações individuais
        if (groupConfig) {
          // Verificar se o auto-responder está ativo NESTE grupo específico
          if (!groupConfig.is_enabled) {
            console.log('🚫 Auto-responder desativado NESTE grupo:', groupData?.name);
            return new Response(JSON.stringify({ success: true, skipped: 'group_autoresponder_disabled' }), {
              headers: { ...corsHeaders, 'Content-Type': 'application/json' }
            });
          }
          
          // Se config_type for 'disabled', ignorar
          if (groupConfig.config_type === 'disabled') {
            console.log('🚫 Grupo com config_type disabled:', groupData?.name);
            return new Response(JSON.stringify({ success: true, skipped: 'group_config_disabled' }), {
              headers: { ...corsHeaders, 'Content-Type': 'application/json' }
            });
          }
          ...
          // Verificar respond_all primeiro (se true, responde tudo)
          ...
          // Se respond_on_mention está ativo mas não foi mencionado, e respond_all está desativado
          if (groupConfig.respond_on_mention && !groupConfig.respond_all && !isMentioned && !isQuestion) {
            console.log('🤫 Silêncio no grupo: IA não mencionada e respond_on_mention ativo:', groupData?.name);
            return new Response(JSON.stringify({ success: true, skipped: 'group_no_mention' }), {
              headers: { ...corsHeaders, 'Content-Type': 'application/json' }
            });
          }
          ...
          console.log(`✅ Grupo ${groupData?.name} - Condições atendidas: mention=${isMentioned}, question=${isQuestion}, respond_all=${groupConfig.respond_all}`);
        } else {
          // Grupo não tem configuração individual - usar padrão
          // Se herdar do PV, permitir responder
      ...
      // Check if it's an audio/ptt message - also check mimetype for audio
      const isAudioByMimetype = mediaMimetype?.toLowerCase().startsWith('audio/') || 
                               ...
                               mediaMimetype?.toLowerCase().includes('mp4'); // WhatsApp sometimes uses audio/mp4
      const isAudioMessage = mediaType === 'audio' || mediaType === 'ptt' || isAudioByMimetype;
      
      // Check if it's image
      const isImageByMimetype = mediaMimetype?.toLowerCase().startsWith('image/');
      const isImageMessage = mediaType === 'image' || isImageByMimetype;
      ...
      // Detectar se é mídia (mais robusto)
      const hasMediaContent = hasMedia || isAudioMessage || isImageMessage || 
                             mediaType === 'audio' || mediaType === 'ptt' || mediaType === 'image' ||
                             mediaMimetype?.startsWith('audio/') || mediaMimetype?.startsWith('image/');
      ...
      const autoEnabled = settingsMap['wa_auto_enabled'] === 'true';
      const autoMode = settingsMap['wa_auto_mode_default'] || 'human';
      const modoInteligente = settingsMap['wa_shadow_mode_enabled'] === 'true';

      if (!autoEnabled || autoMode === 'human' || autoMode === 'paused') {
        console.log('Auto-responder disabled or paused');
        return new Response(JSON.stringify({ success: true }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
      }
