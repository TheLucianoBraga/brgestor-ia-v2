...
export function useContentPosts(category?: string) {
  ...
  const createPost = useMutation({
    mutationFn: async (input: ContentPostInput) => {
      // Sanitize input - remove empty strings and null values
      ...
      console.log('Creating content post:', sanitizedInput);
    ...
    onError: (error: any) => {
      console.error('Content post mutation error:', error);
  ...
  const updatePost = useMutation({
    mutationFn: async ({ id, ...input }: ContentPostInput & { id: string }) => {
      ...
      console.log('Updating content post:', id, sanitizedInput);
