  ...
  const handleGenerateQRCode = async () => {
    ...
    try {
      ...
      if (result.error === 'already_connected') {
        setConnectionStatus('connected');
        if (result.data?.me) {
          setConnectedPhone(result.data.me.id?.split('@')[0] || '');
          setConnectedName(result.data.me.pushName || '');
        }
      ...
      if (result.success && result.data?.value) {
        let qrValue = result.data.value;
        
        // Handle raw QR code (text value for external QR generation)
        if (qrValue.startsWith('raw:')) {
          const rawValue = qrValue.substring(4);
          setQrCode(`https://api.qrserver.com/v1/create-qr-code/?size=256x256&data=${encodeURIComponent(rawValue)}`);
        } else {
          // It's base64 image
          const base64Data = qrValue.startsWith('data:') ? qrValue : `data:image/png;base64,${qrValue}`;
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (status === 'WORKING') {
          ...
          setConnectionStatus('waiting_qr');
          // Try to get QR
          const qrResult = await callWahaApi('get-qr');
          if (qrResult.success && qrResult.data?.value) {
            setQrCode(`data:image/png;base64,${qrResult.data.value}`);
          }
        } else {
          setConnectionStatus('disconnected');
          setConnectedPhone('');
