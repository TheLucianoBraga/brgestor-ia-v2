...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`WAHA: ${action} for tenant ${tenantId}`);
...
async function getQRCode(baseUrl: string, apiKey: string, session: string): Promise<WAHAResponse> {
  ...
  try {
    ...
    if (statusRes.ok) {
      ...
      console.log('Session not found, will create');
    }
    ...
    if (sessionStatus === 'SCAN_QR_CODE') {
      ...
      if (qr) return { success: true, data: { value: qr } };
    }
...
async function fetchQR(baseUrl: string, apiKey: string, session: string): Promise<string | null> {
  ...
  try {
    ...
    if (res.ok && res.headers.get('content-type')?.includes('image')) {
      ...
      if (buffer.byteLength > 100) {
        ...
        console.log('QR fetched as image');
        return base64;
      }
    }
  } catch (e) {
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      if (data.data) {
        console.log('QR fetched as JSON base64');
        return data.data;
      }
      if (data.value) {
        console.log('QR fetched as value');
        return data.value;
      }
    }
  } catch (e) {
    console.log('JSON fetch failed:', e);
  }
